...
async function main() {
    ...
    const { fill, receiver, discharge, endoBasin } = hy;
    ...
    for (let p = 0; p < N; p++) {
        ...
        // discharge is monotone downstream except at an endorheic outlet, where the
        // closed basin's water terminates (evaporates) by design rather than spilling
        const endoOutlet = endoBasin[p] !== -1 && endoBasin[r] !== endoBasin[p];
        if (px.landPx[r] && !endoOutlet && discharge[r] + 1e-9 < discharge[p]) dischDown++;
        if (!px.landPx[r]) dischToSea += discharge[p];
    }
    const endoLoss = hy.endorheicLossKm3;
    const massErr = Math.abs((dischToSea + endoLoss) - runoffTotal) / runoffTotal;
    ...
    const rvSea = rv.filter(r => r.terminus === 'sea').length;
    const rvSalt = rv.filter(r => r.terminus === 'salt lake').length;

    // ---- verdicts ----
    const consistency = [
        ['Mass balance — land runoff equals discharge to the sea plus closed-basin loss',
            `${runoffTotal.toFixed(0)} = ${dischToSea.toFixed(0)} to sea + ${endoLoss.toFixed(0)} endorheic km³/yr (error ${(massErr * 100).toFixed(2)}%)`, massErr < 1e-3],
        ...
        ['Discharge is monotone downstream except at endorheic outlets (accumulation conserved)',
            `${dischDown} unexpected violations`, dischDown === 0],
    ];
    ...
    md.push('',
        ...
        `| Major rivers (≥ 15 km³/yr) | ${rv.length} (${rvSea} to sea, ${rvSalt} to salt lakes) | ~50–100 | high — extensive wet land |`,
        '',
    ...
    md.push('', '## Lakes — endorheic-only reporting', '',
        ...
        `Closed basins are **terminal** in the routing: ${endoLoss.toFixed(0)} km³/yr ` +
        `(${(100 * endoLoss / runoffTotal).toFixed(1)}% of land runoff) drains into the ${lk.length} endorheic lakes and ` +
        'evaporates instead of reaching the sea, so no river is drawn crossing or leaving a ' +
        'salt lake (the priority-flood\'s spill gradient would otherwise route the inflow ' +
        'straight over the rim).', '',
        '---', '',
