...
async function main() {
    ...
    // ---- lakes ----
    const lk = hy.lakes;
    const salt = lk.filter(l => l.endorheic), fresh = lk.filter(l => !l.endorheic);
    ...
    const bin = arr => { const e = [2e3, 1e4, 5e4, 2e5, 1e6, Infinity], c = [0, 0, 0, 0, 0];
        ...
        return c; };
    const fDepth = fresh.map(l => l.maxDepthKm).sort((a, b) => a - b);
    ...
    md.push('', '## Lakes — a resolution caveat', '',
        '| lake set | count | area (M km²) | % of land | size bins 2k/10k/50k/200k/1M+ |',
        '|---|---:|---:|---:|---|',
        `| Endorheic (closed-basin) | ${salt.length} | ${(area(salt) / 1e6).toFixed(2)} | ${(100 * area(salt) / landArea).toFixed(1)}% | ${bin(salt).join(' / ')} |`,
        `| Exorheic (filled depressions) | ${fresh.length} | ${(area(fresh) / 1e6).toFixed(2)} | ${(100 * area(fresh) / landArea).toFixed(1)}% | ${bin(fresh).join(' / ')} |`,
        `| **All ≥ 2,000 km²** | **${lk.length}** | **${(area(lk) / 1e6).toFixed(2)}** | **${(100 * area(lk) / landArea).toFixed(1)}%** | |`,
        '',
        ...
        `${fDepth.length ? (fDepth[fDepth.length >> 1] * 1000).toFixed(0) : '0'} m — so it is basin relief, not shallow noise).`,
        '',
        '**Endorheic closure is resolution-robust** (a basin whose inflow cannot outpace',
        `evaporation is genuinely terminal at any resolution), so the **${salt.length} endorheic`,
        `lakes (${(100 * area(salt) / landArea).toFixed(1)}% of land)** are the trustworthy standing-water population,`,
    ...
    console.log(`internal consistency: ${allPass ? 'ALL PASS' : 'FAILURES'}; ` +
        `lakes ${lk.length} (${salt.length} endorheic / ${fresh.length} exorheic)`);
